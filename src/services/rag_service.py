...
class RAGService:
    ...
    def chat(
        ...
        # 4. 构建 prompt
        system_prompt = """你是一位专业的金融行业分析师。请根据提供的上下文片段用流畅的中文，类似研报摘要风格回答用户问题。
        ## 回答要求：
        1. **严禁幻觉**：绝对不要提及上下文中没有出现的公司名称或数据。如果上下文没有提到某公司，就不能在答案中出现该公司。
        2. **严禁使用训练知识**：不要利用你的内部记忆补充任何信息，即使你"知道"某些事实。
        3. **综合叙述**：将同一家公司的信息整合在一起，不要机械罗列每条文档
        4. **穷尽实体**：提取上下文中**所有**被提及的相关公司，包括：
        - 公告发布方（主角）
        - 文中提到的竞争对手、合作伙伴、子公司（配角）
        5. **抓大放小**：如果未提及金额或日期，直接省略，不要写"未提及"
        6. **主次分明**：重点关注文档的"主角"公司；如果是A公司提到的竞品，请明确说明"A公司在报告中分析了..."
        ## 输出格式：
        根据现有公告分析，[主题]的主要动态如下：
        **1. [公司名称] ([核心动作])**
        [2-3句话描述关键信息]
